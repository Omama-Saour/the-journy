...
import { SendJobTitle, SendSkills } from "../../../../modules/steps/stepone/service"; 
...
const Skill = ({ name, isSelected, onToggle }) => (
  ...
  >
    ...
    />
    {isSelected ? (
      <img loading="lazy" src={vec} alt="Selected" className="w-5 aspect-square mr-2" />
    ) : (
...
const NoSuggested = ({ onSave, suggestedSkills, suggestedSkillsAR, jobTitles }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setError("حدث خطأ أثناء حفظ البيانات. تأكد من اختيار مسمى وظيفي ثم حاول مرة أخرى.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <>
      ...
      <section className="fixed bottom-0 left-0 right-0 top-0 flex flex-col justify-center items-center px-5 py-4 z-50">
        <main className="flex relative flex-col justify-center bg-white rounded-3xl min-w-[450px] pt-20 pb-8 ml-20 mr-20 overflow-hidden max-md:px-5 max-md:py-24">
          <h1 className="flex z-0 flex-col w-full p-8 text-xl font-bold leading-none text-center text-neutral-800 max-md:max-w-full">
            بالإضافة إلى المهارات التي اخترتها سابقاً هذه مهارات مقترحة بناء على
            أجوبتك في اختبار تحديد الشخصية
          </h1>
          ...
          <div className="overflow-y-auto w-full px-5">
            ...
            >
              ...
              <div className="flex flex-wrap gap-6 items-center mt-2 mb-20 w-full max-md:max-w-full">
                {jobTitles.length > 0 ? (
                  jobTitles.map((job, index) => (
                    <Job
                      key={index}
                      name={job.job_title_arabic}
                      isSelected={selectedJob === `${job.job_title} (${job.job_title_arabic})`}
                      onToggle={() => handleToggleJob(`${job.job_title} (${job.job_title_arabic})`)}
                    />
                  ))
                ) : (
                  <p className="text-neutral-600">لا توجد مسميات وظيفية مقترحة</p>
                )}
              </div>
