...
import { Get_CV, Enhance_Experience, Enhance_Summery } from "../../../modules/steps/steptwo/service"; 
...
import Vector from "../../../assets/loading/Vector1.png";
...
const ProfileCard = () => {
  ...
  const [enhancementLoading, setEnhancementLoading] = useState(false); // State for enhancement loading
  ...
  const handleGetEnhance = async () => {
    ...
    try {
      ...
      setEnhancementError("حدث خطأ في الحصول على الاقتراحات. يرجى المحاولة مجدداً");
   ...
   if (!profileData) {
    return (
      ...
      <div className="flex justify-center items-center">
        <p dir="rtl" className="text-red-500">خطأ في تحميل البيانات، أعد تحميل الصفحة.</p>
      </div>
      </div>
      </section>
    );
  ...
  const { personal_info, summary, education, experiences, skills, languages, certificates, references } = profileData;
  ...
  <>
    <section className="flex overflow-hidden flex-col justify-start p-8 bg-white min-w-[240px] rounded-[31px] shadow-[8px_8px_24px_rgba(32,32,32,0.1)] w-[796px] max-md:px-5 max-md:max-w-full">
      <div className="flex flex-col w-full max-md:max-w-full">
        ...
        />
        <EditableSection title="ملخص شخصي" editComponent={PersonalSummaryEdit} onRefresh={fetchProfileData} summary={summary.summary}>
          <div className="text-base text-neutral-800">{summary.summary || "لا توجد ملخصات متاحة"}</div>
        </EditableSection>
         <EditableSection title="التعليم" editComponent={EducationForm} onRefresh={fetchProfileData} education={education}>
  {education.map((edu, index) => (
    <div key={index} className="flex flex-col justify-center w-full text-base text-neutral-800 max-md:max-w-full mb-5">
        ...
        <div className="flex flex-col justify-center self-stretch my-auto w-full">
          ...
          <div className="flex items-center mt-2 w-full">
            <div className="self-stretch my-auto text-stone-500">
              {`${edu.university_start_date} - ${edu.university_end_date || 'Present'}`}
            </div>
      ...
      <div className="mt-3 text-right max-md:max-w-full">
        ...
        <EditableSection title="الخبرة" editComponent={ExperienceEdit} onRefresh={fetchProfileData} experiences={experiences} >
      {experiences.map((exp, index) => (
        ...
        <EditableSection title="المهارات" editComponent={SkillesEdit} onRefresh={fetchProfileData} skills={skills}>
      <div className="text-base text-neutral-800">
        ...
        <EditableSection title="اللغات" editComponent={LanguageEdit} onRefresh={fetchProfileData} languages={languages}>
          <div className="text-base text-neutral-800">
            {languages.map((language, index) => (
            <div key={index} className="text-base text-neutral-800">{language.language_name}</div>
          ))}
            </div>
        </EditableSection>
      <EditableSection title="الشهادات" editComponent={CertificateEdit} onRefresh={fetchProfileData} certificates={certificates}>
          {certificates.map((cert, index) => (
            <div key={index} className="text-base text-neutral-800">{`(${cert.certificate_date}) ${cert.certificate_name}`}</div>
          ))}
        </EditableSection>
       <EditableSection title="المراجع" editComponent={ReferencesEdit} onRefresh={fetchProfileData} references={references}>
          {references.map((ref, index) => (
            <div key={index} className="text-base text-neutral-800">{`${ref.first_name} ${ref.last_name} - ${ref.phone}`}</div>
          ))}
        </EditableSection>
      </div>
...
{enhancementError && (
          <div className="text-red-500 text-center mt-4">{enhancementError}</div>
        )}
{enhancementLoading && 
   <Loadding show={enhancementLoading} />
}

{enhancementDone && 
  <EnhanceDisplay show={enhancementDone} data={enhancementData} onClose={onClose}/>
  }
      <style jsx>{`
        .loader {
          border: 4px solid rgba(255, 255, 255, 0.3);
